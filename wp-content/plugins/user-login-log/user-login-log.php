...
if( !class_exists( 'UserLoginLog' ) )
{
 ...
 class UserLoginLog
 {
    ...
    function screen_options()
    {
        ...
        $page = ( isset($_GET['page']) ) ? esc_sql($_GET['page']) : false;
        if( 'login_log' != $page )
            return;
    ...
    */
    ...
    function login_action($user_login)
    {
        ...
        if ( isset( $_REQUEST['redirect_to'] ) ) { $data[$this->data_labels['login Redirect']] = esc_sql( $_REQUEST['redirect_to'] ); }
        $data[$this->data_labels['User Agent']] = esc_sql( $_SERVER['HTTP_USER_AGENT'] );
		...
		$real_client_ip_address = isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? esc_sql($_SERVER['HTTP_X_FORWARDED_FOR']) : esc_sql($_SERVER['REMOTE_ADDR']);
...
?>  <style>
        ...
        echo '<div class="wrap srp">';
            ...
            echo '<div class="tablenav top">';
                ...
                $username = ( isset($_GET['filter']) ) ? esc_sql($_GET['filter']) : false;
            ...
            echo '<div class="tablenav top">';
                ...
                $mode = ( isset($_GET['mode']) ) ? esc_sql($_GET['mode']) : false;
            ...
            if( isset( $_GET['filter'] ) || isset( $_GET['user_role'] ) || isset( $_GET['datefilter'] ) || isset( $_GET['result'] ) )
            {
                ...
                echo '<input type="hidden" name="page" value="login_log" />';
                echo '<input type="hidden" name="download-login-stats" value="true" />';
                echo '<input type="hidden" name="where" value="' . esc_sql(serialize($where)) . '" />';
    ...
    function init_csv_export()
    {
        ...
        $download = (isset($_GET['download-login-stats'])) ? esc_sql($_GET['download-login-stats']) : false;
    ...
    function column_default($item, $column_name)
    {
        ...
        switch($column_name){
            ...
            case 'login_result':
                ...
                return ( $item[$column_name] == '1' ) ? __('Successful', 'ull') : '<div class="login-failed">' . __('Failed', 'ull') . '</div>';
            case 'user_role':
                if( !$item['uid'] )
                    return;
